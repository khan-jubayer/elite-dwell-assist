      ...
      if (loggedUser && loggedUser.email) {
        try {
          ...
          if (Array.isArray(data)) {
            const driverReviews = data.filter(
              (review) => review.driverEmail === loggedUser.email
            );
            ...
            if (driverReviews.length > 0) {
              const totalRating = driverReviews.reduce(
              ...
              try {
                await fetch("http://localhost:5000/averageRatingDriver", {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                  },
                  body: JSON.stringify({
                    driverEmail: loggedUser.email,
